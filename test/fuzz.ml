...
(* Run a program on the emulator and on the simulated hardware, returning `None` if end state is the same
...
let _ =
  for i = 1 to 100 do
    let insn_stream =
      random_stream ~mem_range:Int32.(zero, of_int_exn 16) ~reg_max:8 ~seed:i ()
    in
...
let _ =
  for i = 1 to 100 do
    let insn_stream =
      random_stream ~mem_range:Int32.(min_value, max_value) ~reg_max:32 ~seed:i ()
    in
    let program, trace = generate_program ~insn_count ~insn_stream in
    Stdio.printf "Running program %d\n" i;
    (* List.iter trace ~f:(fun i -> Stdio.print_s (Riscvemulate.sexp_of_insn i));
  Stdio.printf "\nInitial memory contents:\n";
  Stdio.print_s (Hashtbl.sexp_of_t sexp_of_int32 sexp_of_int program); *)
    let result = test_program ~program ~insn_count:(insn_count + 5) in
