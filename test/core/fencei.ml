...
let () =
  ...
  List.iter Test.initial_registers ~f:(fun (address, value) ->
    Cyclesim.Memory.of_bits regfile ~address (Bits.of_int32_trunc ~width:32 value))
;;
...
let rec run cycles =
  ...
  then failwith "Fence.i test timed out"
    ...
    Option.iter (Sim.Cpu.commit_pc sim) ~f:(fun pc ->
      committed_pcs := pc :: !committed_pcs);
    Sim.Cpu.cycle sim;
    run (cycles - 1))
;;
...
let () =
  ...
  if not
       (List.equal
          Int32.equal
          committed_pcs
          [ Int32.zero; Int32.of_int_exn 4; Int32.of_int_exn 8 ])
