...
(** Update CSRs for an sret instruction, returning the PC to resume from. *)
let execute_sret { csrs; privilege; _ } =
  let open Riscv.Csr_address in
  let mstatus_value = csrs.(mstatus) in
...
let csr_access_legal ~privilege ({ op; src; csr; _ } : Riscv.csr) =
  let open Riscv in
   ...
   | Csr ({ op; rd; src; csr } as csr_insn) ->
     ...
     then
       ...
       := take_trap
            ...
            ~trap_value:(Riscv.to_int32 insn)
            ~interrupt:false
     else (
     ...
     let cause =
       Int32.of_int_exn
         (match !privilege with
          ...
          | _ -> 11)
     in
     next_pc := take_trap state ~epc:!pc ~cause ~trap_value:Int32.zero ~interrupt:false
   | Ebreak ->
     next_pc
     := take_trap
          state
   ...
   | Mret ->
     ...
     then next_pc := execute_mret state
       ...
       := take_trap
            ...
            ~interrupt:false
   | Sret ->
     let tsr = Int32.(csrs.(Csr_address.mstatus) land (of_int_exn 1 lsl 22) <> zero) in
     if !privilege >= 1 && ((not tsr) || !privilege = 3)
     then next_pc := execute_sret state
     else
       next_pc
       := take_trap
            state
            ~epc:!pc
            ~cause:(Int32.of_int_exn 2)
            ~trap_value:(Riscv.to_int32 Sret)
            ~interrupt:false);
  (* Preserve r0 *)
  regs.(0) <- Int32.zero;
  (* Update PC *)
